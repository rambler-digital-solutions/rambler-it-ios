...
- (void)setupView {
    ...
    testEvent.name = @"Rambler.iOS #5";
    testEvent.startDate = [NSDate date];
    testEvent.eventDescription = @"Пришла осень, астрологи предсказали пору повышенной активности мобильных разработчиков - и 24 сентября мы готовы провести новую встречу Rambler.iOS официально под номером пять. Для тех, кто нас не помнит, небольшое вступление: мы — команда iOS-разработки холдинга Rambler&Co, любим делиться своими опытом и наработками с сообществом, поэтому организовали нерегулярные митапы для всех заинтересованных. Наше ключевое отличие от прочих кружков по интересам — мы уделяем большее внимание именно качеству выступлений, а не их количеству — все докладчики проходят через многоэтапную систему верификации и прослушивания — и на выходе получается действительно стоящий материал, ради которого не жалко потратить хмурый вечер четверга.";
    testEvent.eventSubtitle = @"Очередная конференция посвященная iOS-разработке";
